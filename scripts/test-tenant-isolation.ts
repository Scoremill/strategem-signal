 ...
 *
 * Creates two test orgs, inserts a tracked_markets row into each, queries
 * each org's data through tenantQuery, and verifies the helper correctly
 * isolates the rows. Cleans up after itself.
 *
...
import { orgs, trackedMarkets, geographies } from "../src/lib/db/schema";
...
async function main() {
  ...
  // injection without colliding with the org_id/geography_id unique index.
  ...
  try {
    // Each org tracks the same geography
    const tA = tenantQuery(orgAId);
    const tB = tenantQuery(orgBId);

    await tA.insert(trackedMarkets, { geographyId: geo.id });
    await tB.insert(trackedMarkets, { geographyId: geo.id });
    ...
    const deleted = await tA.delete(trackedMarkets, eq(trackedMarkets.id, bRowId));
    if (deleted.length !== 0)
    ...
    console.log("Cleaned up test orgs");
  }
}
