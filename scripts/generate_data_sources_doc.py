...
federal_rows = [
    ...
    [
        ...
        "Monthly single-family residential building permits by metro. Primary signal for home-construction demand.",
        "Monthly · 6th @ 11:00 UTC",
        "demand-pipeline.ts · demand-data.yml · /api/cron/demand",
        "permit_data",
        "Filter 1 (Migration + Demand), Portfolio Health Demand sub-score, market narratives",
        "Some metros publish quarterly rather than monthly — permits for those lag by ~30 days",
    ...
    [
        ...
        "income-pipeline.ts · /api/cron/income (manual/annual)",
        "income_data",
        "Filter 5 (Affordability Runway), Portfolio Health Financial sub-score",
        "One-year lag is structural; no fix available. Rural metros have wider confidence intervals.",
        "https://www.census.gov/programs-surveys/acs/",
    ],
    [
        "Census Bureau · Population Estimates Program (PEP)",
        "Annual population change and net domestic migration by metro.",
        "Annual (typ. March release)",
        "demand-pipeline.ts (merged with permits run) · demand-data.yml",
        "migration_data",
        "Filter 1 (Migration + Demand), Portfolio Health Demand sub-score",
        "Annual only; no monthly granularity available from Census",
        "https://www.census.gov/programs-surveys/popest.html",
    ],
    [
        "BLS · Current Employment Statistics (CES)",
        "Total nonfarm employment by metro, month-over-month and year-over-year change.",
        "Monthly · 6th @ 11:00 UTC",
        "demand-pipeline.ts · demand-data.yml · /api/cron/demand (via FRED mirror)",
        "employment_data",
        "Filter 1 (Migration + Demand), Portfolio Health Demand sub-score",
        "FRED mirror publishes ~15 days after BLS; we accept that lag in exchange for rate limits",
    ...
    [
        ...
        "Monthly · 6th @ 11:00 UTC",
        "demand-pipeline.ts · demand-data.yml · fred-client.ts",
        "employment_data (same table as CES)",
        "Portfolio Health Demand sub-score (inverted)",
        "Same FRED mirror lag as CES",
        "https://www.bls.gov/lau/",
    ],
    [
        "BLS · Quarterly Census of Employment & Wages (QCEW)",
        "Construction-trade employment (NAICS 2382/2383/2389) and avg weekly wage by metro. Primary capacity signal.",
        "Quarterly · 15th of Jan/Apr/Jul/Oct @ 12:00 UTC",
        "capacity-pipeline.ts · capacity-data.yml · /api/cron/capacity",
        "trade_capacity_data",
        "Business Case Organic model (wage-adjusted build cost), Filter 6 (Operational Feasibility), Portfolio Health Operational sub-score",
        "~2-quarter publication lag is structural; single-probe resolution added in Phase 3.1",
...
commercial_rows = [
    [
        ...
        "4 markets unmatched of 199 (98% coverage): 2 Puerto Rico metros, Dayton-Kettering OH, Prescott Valley-Prescott AZ. No Zillow coverage at all for those.",
        "https://www.zillow.com/research/data/",
    ],
    [
        "FRED (St. Louis Fed)",
        "Mirror of BLS CES, LAUS, and related employment/population time series. We use FRED because its API is well-documented and rate-friendly.",
        "As-needed (demand-pipeline uses it for CES + LAUS)",
        "fred-client.ts (library used by demand-pipeline)",
        "employment_data",
        "Indirect — FRED is the transport; underlying source is BLS",
        "FRED only covers ~60 top metros directly; small metros must go straight to Census BPS",
...
gap_rows = [
    [
        ...
        "Aguadilla-Isabela PR, San Juan-Carolina-Caguas PR, Dayton-Kettering OH, Prescott Valley-Prescott AZ. No Zillow coverage at all.",
        "Business Case Organic model cannot compute for these markets (returns empty result with a warning).",
        "Phase 4 — source a PR-specific + Ohio-specific home-value feed, or document as out-of-scope.",
    ],
    [
        ...
        "Coverage footer on the Competitive Landscape card flags the gap. Card is public-only by design in v1.",
        "Phase 4.13 — build private-builder pipeline (NAHB Builder 400 paid feed, Zonda, or county-permit scraping).",
    ],
    [
        "Acquisition cost modeling",
        "Current v1 shows the Competitive Landscape as a list; no per-builder cost math. The v1 Acquisition Targets card is just a list — the CEO decides suitability.",
        "Clearly labeled. Prior 'goodwill premium per unit' number was retired in Phase 3.10b as misleading.",
        "Phase 4.14 — rebuild Acquisition Entry model with acquirer-vs-target scale logic, book value, integration cost. Depends on 4.13.",
    ],
    [
        "OEWS coverage limits",
        "Only ~7 of 52 tracked markets have OEWS data at the metro level.",
        "Filter 6 uses QCEW as primary and falls back to OEWS where available; scoring is robust to OEWS gaps.",
    ...
    [
        "FRED / CES / LAUS mirror lag",
        "~15-day delay vs direct BLS.",
        "Acceptable for monthly cadence.",
        "Accept — rate limits and reliability are worth the 2-week lag.",
    ],
    [
        "EDGAR parsing — lot data",
        "Some builders disclose lot positions only in 10-K (annual), not 8-K (quarterly). Phase 3 known gap.",
        "Competitive Landscape data is narrative-derived and unaffected. Phase 3.11 Acquisition rebuild will need to handle this.",
    ...
    [
        ...
        "Not currently surfaced in StrategemSignal; will be upgraded before the feature lands.",
